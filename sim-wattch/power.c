...
#define LowSwingPowerfactor (Mhz)*.2*.2f
...
void dvfs_controller(){
  ...
  static double diff_avg_trgt_pwr;
	if (file_opened == 0) {
		file_opened = 1;
    ...
    diff_avg_trgt_pwr=(dvfs_target_power - avg_power_per_interval)/(dvfs_target_power + avg_power_per_interval);
		/* cs203A FSF's value is varied between 0.2 and 2.0.*/
		if((FSF+diff_avg_trgt_pwr) <= 1.8){
			FSF += diff_avg_trgt_pwr;
		}

		/* cs203A VSF's value is varied between 0.2 and 2.0.*/
		if((VSF+diff_avg_trgt_pwr) <= 1.8){
			VSF += diff_avg_trgt_pwr;
		}
	...
	/* cs203A if average power in the last interval is greater than DVFSTargetPower, then the FSF and VSF is decremented by 0.2*/
     ...
     diff_avg_trgt_pwr=(avg_power_per_interval - dvfs_target_power) / (dvfs_target_power + avg_power_per_interval);
		/* cs203A FSF's value is varied between 0.2 and 2.0.*/
		if((FSF - diff_avg_trgt_pwr) >= 0.4){
			FSF -= diff_avg_trgt_pwr;
		}

		/* cs203A VSF's value is varied between 0.2 and 2.0.*/
		if((VSF - diff_avg_trgt_pwr) >= 0.4){
			VSF -= diff_avg_trgt_pwr;
		}
		calculate_power(&power);
	}
